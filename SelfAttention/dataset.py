...
class SteamboatDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return sample['X'], sample['Morpho'], sample['adj']

    

def prep_adatas(adata: sc.AnnData, n_neighs: int = 8, norm=True, log1p=True, scale=False, renorm=False) -> list[sc.AnnData]:
...
def make_dataset(adata: sc.AnnData, sparse_graph=True, mask_var: str = None, obsm_key = None) -> SteamboatDataset:
    ...
    if mask_var:
        assert mask_var in adata.var.columns, f"Not all adatas have {mask_var} in var"

        temp = adata.var[mask_var]
        assert (adata.var[mask_var] == temp).all(), f"Not all adatas have {mask_var} in var"
