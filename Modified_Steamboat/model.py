...
class HadmardAttention(nn.Module):
    def __init__(self, d_in: int, d_in_M: int, n_heads: int, model_type: int=0,  n_scales: int = 2, d_out: int = None, print_flag=False):
        ...
        self.encoder = Encoder(input_dim=d_in_M, output_dim=d_in_M)
        #self.encoder = nn.Linear(d_in + d_in_M, self.d_out)

        self.q = NonNegLinear(self.d_out, n_heads, bias=False)
        self.k = NonNegLinear(self.d_out, n_heads, bias=False)
        self.k_local = NonNegLinear(self.d_out, n_heads, bias=False)
