...
def calc_head_weights(device, adatas, model):
    ...
    for i in range(len(adatas)):
        ...
        morpho += np.mean(adatas[i].obsm['morpho_attn'], axis=0)
        local += np.mean(adatas[i].obsm['local_attn'], axis=0)
        regional += np.mean(adatas[i].obsm[f'regional_attn_{i}'], axis=0)

    matrix = np.vstack([ego, morpho, local, regional]) * calc_v_weights(device, model)
...
def plot_head_weights(head_weights, multiplier: float = 100, order=None, figsize=(10, 1), heatmap_kwargs=None, save: str = None):
    ...
    ax.set_yticklabels(['ego', 'morpho', 'local', 'regional'], rotation=0)
...
def calc_obs(adatas: list[sc.AnnData], dataset: 'SteamboatDataset', model: 'Steamboat',
    ...
    for i, (x, m, adj_list, regional_xs, _, regional_adj_lists) in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        with torch.no_grad():
            res, details = model(adj_list, x, m, x, regional_adj_lists, regional_xs, get_details=True)

            if get_recon:
                adatas[i].obsm['X_recon'] = res.cpu().numpy()

            adatas[i].obsm['q_score'] = details['embq'].cpu().numpy()
            adatas[i].obsm['k_morpho_score'] = details['embmorpho'].cpu().numpy()
            ...
            adatas[i].obsm['morpho_attn'] = details['attnm'][1].cpu().numpy()
            adatas[i].obsm['local_attn'] = details['attnm'][2].cpu().numpy()
            for j, matrix in enumerate(details['attnm'][3]):
                adatas[i].obsm[f'regional_attn_{j}'] = matrix.cpu().numpy()

            # local attention (as graph)
            for j in range(model.spatial_gather.n_heads):
                w = details['attnp'][2].cpu().numpy()[:, j, :].flatten()
                ...
                adatas[i].obsp[f'local_attn_{j}'] = sp.sparse.csr_matrix((w, (u, v)),
                                                                         shape=(adatas[i].shape[0],adatas[i].shape[0]))
...
def gather_obs(adata: sc.AnnData, adatas: list[sc.AnnData]):
    ...
    for i in range(len(adatas)):
        ...
        all_embmorpho.append(adatas[i].obsm['k_morpho_score'])
        all_embk.append(adatas[i].obsm['k_score'])
        all_ego_attn.append(adatas[i].obsm['ego_attn'])
        all_morpho_attn.append(adatas[i].obsm['morpho_attn'])
    ...
    adata.obsm['k_morpho_score'] = np.vstack(all_embmorpho)
    adata.obsm['k_score'] = np.vstack(all_embk)
    adata.obsm['ego_attn'] = np.vstack(all_ego_attn)
    adata.obsm['morpho_attn'] = np.vstack(all_morpho_attn)
...
def plot_cell_type_enrichment(all_adata, adatas, score_dim, label_key, select_labels=None,
    ...
    all_adata.obsm[f'global_attn_{score_dim}'] = np.vstack([i.obsm['global_attn'][:, None, score_dim] for i in adatas])
