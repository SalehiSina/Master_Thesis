...
class SteamboatDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return sample['X'], sample['Morpho'], sample['adj'], sample['regional_Xs'], sample['regional_Morpho'], sample['regional_adjs']
    ...
    def to(self, device):
        ...
        for sample in self.data:
            ...
            new_sample['Morpho'] = sample['Morpho'].to(device)
            new_sample['adj'] = sample['adj'].to(device)
            new_sample['regional_Xs'] = [j.to(device) for j in sample['regional_Xs']]
            new_sample['regional_Morpho'] = [j.to(device) for j in sample['regional_Morpho']]
...
def make_dataset(adatas: list[sc.AnnData], sparse_graph=True, mask_var: str = None, obsm_key = None,
    ...
    for i in tqdm(range(len(adatas))):
        ...
        if obsm_key is None:
            X = adata.X
            M = adata.obsm['p_Morpho_Embedding']
        else:
            X = adata.obsm[obsm_key[0]]
            M = adata.obsm[obsm_key[0]]
        ...
        if isinstance(X, sp.sparse.spmatrix):
            ...
            data_dict['Morpho'] = torch.from_numpy(M.astype(np.float32))
        else:
        ...
        for key in regional_obs:
            ...
            if unique_values.shape[0] != 1:
                ...
                temp_M = data_dict['Morpho'].mean(axis=0, keepdim=True)
            ...
            data_dict['regional_Morpho'].append(temp_M)
